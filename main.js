$( document ).ready(function(){	
	function galeryOne(){
		$('#slideContainer').animate({"margin-left": 0}, 2000);
	}
	function galeryTwo(){
		$('#slideContainer').animate({"margin-left": "-100%"}, 2000);
	}
	function galeryThree(){
		$('#slideContainer').animate({"margin-left": "-200%"}, 2000);
	}
	$('#showFirst').click(galeryOne);
	$('#showSecond').click(galeryTwo);
	$('#showThird').click(galeryThree);

var x1 = setInterval(function(){
	setTimeout(galeryOne,3000);
},10000);

var x2 = setInterval(function(){
	setTimeout(galeryTwo,6000);
},10000);

var x3 = setInterval(function(){
	setTimeout(galeryThree,9000);
},10000);
...
// current underline of a link in a nav bar. Doesn't work cause we have no other pages
...
function initialize() {
    ...
    for( i = 0; i < markers.length; i++ ) {
        ...
        marker = new google.maps.Marker({
            position: position,
            map: map,
            icon: 'images/ic_room_black_48px.png',
            title: markers[i][0]
        });
